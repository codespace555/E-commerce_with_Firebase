    ...
    <Layout>
      <div className=" bg-gray-100 pt-5 dark:bg-[#282c34]  h-auto">
        ...
        <div className="rounded-lg md:w-2/3 ">
            ...
            cartitem.length===0?<p className='text-center'>Your Cart is Empty! Please add some items to your cart.</p>:<> {
              cartitem?.map((item,index) => (
                <div key={index}>
                  ...
                  <CartItem title={item.title} image ={item.imageurl} price={item.price} describe={item.discription} slug={item.slug} deletecart={() => deleteCart(item)}/>
          ...
          <div className={`mt-6 h-full rounded-lg border bg-white p-6 shadow-md md:mt-0 md:w-1/3  dark:text-gray-600 text-black ${cartitem.length===0? "hidden":"block"}`}>
            ...
            <div className={`flex justify-between`}>
              <p className={`text-gray-700 dark:text-gray-600 ${totalAmount > 300?"hidden":"block"}`}>Shipping</p>
              <p className={`text-gray-700 dark:text-gray-600 ${totalAmount > 300?"hidden":"block"}`}>₹20</p>
            </div>
            <hr className="my-4" />
